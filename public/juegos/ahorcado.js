...
async function finalizarNivel(victoria) {
    ...
    if (victoria) {
        ...
        fetch('/api/mascota/jugar', {
            ...
            body: JSON.stringify({ experienciaGanada: 60 })
        })
        .then(res => res.json())
        .then(data => {
            if (data.hongosGanados) {
                ...
                modalPremio.innerHTML = `
                    <div style="background:#1e1e2f; border:4px solid #ffeb3b; padding:40px; border-radius:20px; text-align:center; box-shadow:0 0 20px #ffeb3b; max-width:320px;">
                        <h2 style="color:#ffeb3b; margin:0 0 10px 0; font-size:1.8rem;">¡NIVEL COMPLETADO!</h2>
                        <p style="color:#fff; margin-bottom:15px;">Has aportado felicidad a tu mascota y ganaste:</p>
